...
function displayRoyalLogin(userElement, item) {
  if (item.toggled) return;

  item.toggled = true;
  const userPic = userElement
    .find(".u-pic")
  ...
  const imgContainer = $(body).append(`
    <div>
      <img class='ro_te3b_img' 
           style='position: absolute; top: 3rem; left: -300px; border-radius: 50%' 
           width='78' 
           height='78' 
           src='${userPic}' />
      <img class='ro_te3b_img' 
           style='position: absolute; top: 1rem; left: -300px; border-radius: 0%' 
           width='329' 
           height='173' 
           src='${item.icon}' />
    </div>
